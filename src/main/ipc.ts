...
export async function registerIpcHandlers(): Promise<void> {
  ...
  ipcMain.handle('grub:set', async (_, opts: Record<string, string>) => {
    ...
    for (const [k, v] of Object.entries(opts)) {
      ...
      // /etc/default/grub is sourced as a shell script by update-grub. Every value is
      // written back double-quoted, so reject the characters that stay meaningful even
      // inside double quotes (backtick/$ for substitution, " to break out, \ to escape,
      // newlines to smuggle in an extra line) before it ever reaches the root helper.
      if (value.length > 512 || /[`$"\\\r\n]/.test(value)) {
        throw new Error(`Invalid value for ${k}`)
      }
      if (k === 'GRUB_TIMEOUT' && !/^-?\d+$/.test(value)) {
        throw new Error('GRUB_TIMEOUT must be an integer')
      }
      if (k === 'GRUB_TIMEOUT_STYLE' && !['menu', 'hidden', 'countdown'].includes(value)) {
  ...
  ipcMain.handle('hosts:write', async (_, content: string) => {
    ...
    // Piped over stdin rather than a shared /tmp file, so there's nothing on disk for a
    // local process to race between our write and the root helper's read.
    return privilegedOpWithStdin('hosts-write', [], content)
  })
  ...
  ipcMain.handle('dns:setGlobal', async (_, servers: string[], search: string[]) => {
    ...
    return privilegedOpWithStdin('resolv-write', [], content)
  })
